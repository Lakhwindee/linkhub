                ...
                <Card className="p-4">
                  <div className="space-y-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <Label className="text-base font-semibold">Campaign Targeting</Label>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        {watchedValues.countries && watchedValues.countries?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {watchedValues.countries?.map((country, index) => (
                              <Badge key={index} variant="outline" className="text-xs">
                                {country}
                                <button 
                                  type="button"
                                  onClick={() => {
                                    const updated = watchedValues.countries?.filter(c => c !== country) || [];
                                    setValue("countries", updated);
                                  }}
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        {...register("hashtags", {
                          setValueAs: (value: string) => 
                            value ? value.split(',').map(tag => tag.trim().replace(/^#/, '')).filter(Boolean) : []
                        })}
                        placeholder="travel, adventure, explore (separate with commas)"
                      />
                      {errors.hashtags && (
                      ...
                      {watchedValues.hashtags && watchedValues.hashtags?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {watchedValues.hashtags?.map((tag, index) => (
